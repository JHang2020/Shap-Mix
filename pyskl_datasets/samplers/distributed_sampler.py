...
class ClassSpecificDistributedSampler(_DistributedSampler):
    ...
    def __init__(self,
        ...
        if class_prob is not None:
            ...
            cls_num = dataset.num_classes

            if dataset.split == 'xset_train':
                skeleton_max = 400
                imb_factor = 0.01
            elif dataset.split == 'train':
                skeleton_max = 100
                imb_factor = 0.1
            else:
                skeleton_max = 600
            ...
            the_cls_tail = int(cls_num / 3) * 2
            for cls_idx in range(cls_num):
                if cls_idx >= the_cls_tail: 
                    num = skeleton_max * (imb_factor**(cls_idx / (cls_num - 1.0)))
                    prob = skeleton_max / int(num)
