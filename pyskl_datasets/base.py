...
from.core import mean_average_precision, mean_class_accuracy, top_k_accuracy, per_class_accuracy
...
class BaseDataset(Dataset, metaclass=ABCMeta):
    ...
    def evaluate(self,
        ...
        allowed_metrics = ['top_k_accuracy', 'mean_class_accuracy', 'mean_average_precision', 'per_class_accuracy']
        ...
        for metric in metrics:
            ...
            if metric == 'per_class_accuracy':
                ...
                per_class = per_class_accuracy(results, gt_labels)

                log_per = []
                class_index = 1
                for per_acc in per_class:
                    log_per.append(f'\nA{class_index}:\t{per_acc:.4f}')
                    class_index = class_index + 1
                log_per = ''.join(log_per)
                print_log(log_per, logger=logger)
    ...
    def prepare_train_frames(self, idx):
        ...
        #print(res['keypoint'][0].shape)#[2, 64, 17, 2] M T V C
        kp = res['keypoint'][0].permute(3,1,2,0)
        return kp, res['label'], -1
    ...
    def prepare_test_frames(self, idx):
        ...
        res =  self.pipeline(results)
        #print( res['keypoint'].shape)
        return res['keypoint'][0].permute(3,1,2,0), res['label'], -1
