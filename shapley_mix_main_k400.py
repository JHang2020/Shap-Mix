...
def get_parser():
    ...
    parser.add_argument(
        '--dataset', default='k400', help='must be train or test')
    parser.add_argument(
        '--save-score',
        type=str2bool,
        default=False,
    ...
    parser.add_argument(
        '--cos', type=str2bool, default=False, help='use nesterov or not')
    parser.add_argument(
...
class Processor():
    ...
    def adjust_learning_rate_cos(self, epoch, idx):
        if self.arg.optimizer == 'SGD' or self.arg.optimizer == 'Adam':
            if epoch < self.arg.warm_up_epoch:
                lr = self.arg.base_lr * (epoch + 1) / self.arg.warm_up_epoch
            else:
                ...
                eta_min = self.arg.base_lr * 0.001
    ...
    def train(self, epoch, save_model=False):
        ...
        label_dist = torch.from_numpy(label_dist).cuda()
        self.adatemp = [0.2,]*23 + [0.3,]*18 + [0.4,]*19
        self.adatemp = torch.tensor(self.adatemp).cuda() 
        if epoch == 1:
            self.avg_motion /= len(loader.dataset)
        for batch_idx, (data, label, index) in enumerate(process):
            if self.arg.cos:
                self.adjust_learning_rate_cos(epoch, batch_idx)
            self.global_step += 1
            with torch.no_grad():
            ...
            if epoch==0:
                self.avg_motion += data.sum(dim=0)
            ...
            if epoch < self.arg.naive_estimate_epoch:
                weight = weight_calcu_k400(model=self.model, data=data, label=label, spa_mask_list=spa_list, average_motion=self.avg_motion).cuda()
            else:
                weight = weight_calcu_shapley_k400(model=self.model, data=data, label=label, spa_mask_list=spa_list, average_motion=self.avg_motion).cuda()
               
            part_each_num = np.array([5,6,6,4,4])#the joint num of each parts
            weight = weight/part_each_num[spa_list].sum()*10 # node normlization
            self.update_part_dist(epoch, self.global_step, spa_label, weight, label)
            ##########################################################

            # mix generation
            if epoch < self.arg.naive_mix_epoch:
                randidx, mix_data, mask = ske_swap_randscale_k400(data,2, 3, 13, 20) #9 15 for clip_len = 80
                lamb = mask.mean()
            else:
                part_dist = self.part_distri[label] 
                randidx, mix_data, lamb = ske_swap_randscale_sample_noweighted_k400(data,2, 3, 13, 20, part_dist, label_dist[label])#, adatemp)
    ...
    def eval(self, epoch, save_score=False, loader_name=['test'], wrong_file=None, result_file=None):
        ...
        for ln in loader_name:
            ...
            accuracy = ((np.concatenate(pred_list) == np.concatenate(label_list))).astype(np.float).mean()
            #self.data_loader[ln].dataset.top_k(score, 1)
    ...
    def start(self):
        if self.arg.phase == 'train':
            ...
            self.avg_motion = torch.zeros(3,100,17,2).cuda()
